	...
	public function movie_room_transaction_listing(){
		...
		$aColumns = array(	
			...
			{   	
				...
				// query for no of persons in a room
				$query_no_of_person = "SELECT SUM(additional_person) FROM additional_mrt_person WHERE mrt_id=".$aRow['idx'];
				$result = mysql_query($query_no_of_person);
				
				$additionalResult = mysql_fetch_array($additional);
				$result_no_of_person = mysql_fetch_array($result);
				...
				$row[] = $aRow['created_by'];
				// $row[] = $aRow['check_out'];
				$row[] = $aRow['no_of_person'] + $result_no_of_person[0];
				...
				$total_no_of_person += $aRow['no_of_person'] + $result_no_of_person[0];
				$i++;
				$output['aaData'][] = $row;
			}
			$date = date("Y-m-d", strtotime($_GET['date_from']));
